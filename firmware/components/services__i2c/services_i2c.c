...
static void worker(void* arg)
{
    ...
    for (;;) {
        req_node_t* n = NULL;
        if (xQueueReceive(s_q, &n, portMAX_DELAY) != pdTRUE || !n) continue;
        ...
        switch (n->r.op) {
            ...
            case I2C_OP_RX:
                ...
                break;
            case I2C_OP_TXRX:
                err = i2c_txrx(n->r.dev, n->tx_copy, n->r.txlen,
                               n->rx_stage, n->r.rxlen, n->r.timeout_ms);
                if (err == ESP_OK && n->r.rx && n->r.rxlen)
                    memcpy(n->r.rx, n->rx_stage, n->r.rxlen);
                break;
            default:
        ...
        /* sprzątanie */
        if (n->tx_copy)  free(n->tx_copy);
        if (n->rx_stage) free(n->rx_stage);
        free(n);
    }
}
...
{
    if (s_q) return true; /* już działa */
    if (queue_len <= 0) queue_len = 16;
    s_q = xQueueCreate(queue_len, sizeof(req_node_t*));
    if (!s_q) return false;

    if (xTaskCreate(worker, "i2c_svc", task_stack > 0 ? task_stack : 4096,
                    NULL, task_prio > 0 ? task_prio : 8, &s_th) != pdPASS) {
...
{
    if (!s_q || !req || !req->dev) return false;
    ...
    if (req->tx && req->txlen) {
        ...
        if (!n->tx_copy) { free(n); return false; }
    ...
    if (req->rx && req->rxlen) {
        ...
        if (!n->rx_stage) { if (n->tx_copy) free(n->tx_copy); free(n); return false; }
    }
