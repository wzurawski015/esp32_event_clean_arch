...
bool services_ds18_start(const ds18_svc_cfg_t* cfg)
{
    ...
    const gpio_config_t io = {.pin_bit_mask = (1ULL << s_gpio),
    ...
    ESP_LOGI(TAG, "DS18B20 service started on GPIO%d, res=%db, period=%dms", s_gpio, s_res_bits, s_period_ms);
    return true;
}
