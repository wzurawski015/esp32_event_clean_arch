...
#undef  LCD_BURST
#define LCD_BURST \
  ((CONFIG_APP_LCD_BURST_SIZE) < 1 ? 1 : \
  ((CONFIG_APP_LCD_BURST_SIZE) > 64 ? 64 : (CONFIG_APP_LCD_BURST_SIZE)))
...
/* -------------------------------------------------------------------------- */
static inline bool has_rgb(void) { return s_dev_rgb != NULL; }

static bool rgb_init_sequence(void) {
    if (!has_rgb()) return true; /* brak RGB → traktujemy jako sukces */
...
static bool lcd_init_next_step(uint8_t *out_delay_ms, bool *out_finished) {
    ...
    *out_finished = false;
    ...
    switch (idx) {
        ...
        case 5:  if (!lcd_cmd(0x6C)) return false; *out_delay_ms = 220; idx++; return true; /* Follower ON (potrzebuje ~220 ms) */
        ...
        case 8:  if (!lcd_cmd(0x01)) return false; *out_delay_ms = 3;   idx++; return true; /* Clear (>=1.64 ms) */
...
static void task_ev(void* arg) {
    ...
    for (;;) {
        ...
        if (m.src == EV_SRC_SYS && m.code == EV_SYS_START) {
            ...
            rgb_set((uint8_t)CONFIG_APP_RGB_R,
                    (uint8_t)CONFIG_APP_RGB_G,
                    (uint8_t)CONFIG_APP_RGB_B);
        }
...
bool lcd1602rgb_init(const lcd1602rgb_cfg_t* cfg) {
    if (!cfg || !cfg->dev_lcd) return false;      /* RGB opcjonalne */
