...
port_err_t i2c_bus_create(const i2c_bus_cfg_t* cfg, i2c_bus_t** out_bus)
{
    ...
    bus->hbus   = hbus;
    bus->clk_hz = (cfg->clk_hz != 0) ? cfg->clk_hz : 100000;
...
{
    if (!bus || !out_dev) return PORT_ERR_INVALID_ARG;

    i2c_dev_t* dev = (i2c_dev_t*)calloc(1, sizeof(i2c_dev_t));
    if (!dev) return PORT_FAIL;
...
{
    if (!dev || (txlen && !tx)) return PORT_ERR_INVALID_ARG;
...
{
    if (!dev || (rxlen && !rx)) return PORT_ERR_INVALID_ARG;
...
port_err_t i2c_bus_probe_addr(i2c_bus_t* bus, uint8_t addr7, uint32_t timeout_ms, bool* out_ack)
{
    if (!bus) return PORT_ERR_INVALID_ARG;
    if (out_ack) *out_ack = false;
    if (addr7 < 0x03 || addr7 > 0x77) return PORT_ERR_INVALID_ARG;

    // Nowe API IDF 5.x – szybkie sondowanie na uchwycie magistrali.
    esp_err_t err = i2c_master_probe(bus->hbus, addr7, timeout_ms);
    if (err == ESP_OK) {
        if (out_ack) *out_ack = true;
        return ESP_OK;
    }
    // Brak ACK traktujemy jako "adres pusty" bez błędu krytycznego:
    if (err == ESP_ERR_TIMEOUT || err == ESP_FAIL) {
        if (out_ack) *out_ack = false;
        return ESP_OK;
    }
    return err; // inne błędy sterownika
}
...
{
    if (!bus) return PORT_ERR_INVALID_ARG;
    if (start > end) return PORT_ERR_INVALID_ARG;

    size_t n = 0;
    for (uint8_t a = start; a <= end; ++a) {
        bool ack = false;
        port_err_t e = i2c_bus_probe_addr(bus, a, timeout_ms, &ack);
        if (e != ESP_OK) return e;
        if (ack && out_found && n < cap) {
            out_found[n++] = a;
        }
    }
    if (out_n) *out_n = n;
