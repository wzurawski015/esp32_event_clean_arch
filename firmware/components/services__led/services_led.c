    ...
    for (;;) {
        ...
        if (xQueueReceive(s_q, &msg, pdMS_TO_TICKS(1000)) == pdTRUE) {
            ...
            if (msg.code == EV_LED_SET_RGB) {
                ...
                if (led_strip_refresh(s_strip, 100) != PORT_OK) {
                ...
                led_strip_set_pixel(s_strip, 0, 0, 0, 20); // Blue, low brightness
                led_strip_refresh(s_strip, 100);
            }
...
bool services_led_start(const ev_bus_t* bus, const led_svc_cfg_t* cfg)
{
    ...
    led_strip_cfg_t port_cfg = {
        ...
        .type     = (led_type_t)cfg->led_type, // Rzutowanie bezpieczne, jeśli enumy są zgodne
        .use_dma  = true // Dla C6 DMA jest zalecane
    };

    if (led_strip_create(&port_cfg, &s_strip) != PORT_OK) {
    ...
    if (!ev_bus_subscribe(s_bus, &s_q, 8)) { // Kolejka o głębokości 8 wystarczy dla LED
        ...
        led_strip_delete(s_strip);
        return false;
    }

    /* 3. Start Workera */
    /* Stack 2048-3072B zazwyczaj wystarcza, chyba że używasz printf w tasku */
    BaseType_t ret = xTaskCreate(led_task, "svc_led", 3072, NULL, tskIDLE_PRIORITY + 1, &s_task);
    if (ret != pdPASS) {
        LOGE(TAG, "Task create failed");
        ev_bus_unsubscribe(s_q); // Cleanup queue is automatic? W Twoim API unsubscribe nie usuwa kolejki, trzeba ręcznie?
        // Zgodnie z Twoim core_ev.c: ev_subscribe tworzy kolejkę.
        // Tutaj dla uproszczenia zakładamy sukces lub panic w init.
        return false;
    }
